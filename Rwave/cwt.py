...
def cwt(input, noctave, nvoice=1, w0=2 * np.pi, twoD=True, plot=False):
    ...
    scales_per_octave = nvoice
    coefs_3d = np.empty((len(input), noctave, nvoice))
    for i in range(noctave):
        start = i * scales_per_octave
        end = (i + 1) * scales_per_octave
        coefs_3d[:, i, :] = coefs[:, start:end]

    cwt_result = coefs_3d

    if twoD:
        cwt_result = np.transpose(coefs)


        if plot:
            plt.imshow(cwt_result, aspect='auto', cmap='jet')
            plt.xlabel("Time")
